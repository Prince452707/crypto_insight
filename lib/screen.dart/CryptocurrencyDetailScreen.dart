...
class _CryptocurrencyDetailScreenState extends State<CryptocurrencyDetailScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: 5, vsync: this);
    _scrollController = ScrollController();
    _loadBookmarkStatus(); // Load bookmark status on init
    fetchDetails();
  }
  ...
  Future<void> _loadBookmarkStatus() async {
    ...
    setState(() {
      isBookmarked = bookmarkedIds.contains(widget.cryptocurrency.id);
    });
  }
  ...
  Future<void> _toggleBookmark() async {
    ...
    if (isBookmarked) {
      ...
      bookmarkedIds.add(widget.cryptocurrency.id);
    }

    await prefs.setStringList('bookmarked_crypto_ids', bookmarkedIds);
    setState(() {
      isBookmarked = !isBookmarked;
    });
