...
class CryptocurrencyOverviewTab extends StatelessWidget {
  ...
  const CryptocurrencyOverviewTab({
    Key? key,
        ...
        child: Column(
          ...
          children: [
            ...
            _buildStatItem('Market Cap',
                '\$${NumberFormat("#,##0.00").format(cryptocurrency.marketCap)}'),
            _buildStatItem('24h Volume',
                '\$${NumberFormat("#,##0.00").format(cryptocurrency.volume24h)}'),
            _buildStatItem(
                'Circulating Supply',
                '${NumberFormat("#,##0.00").format(cryptocurrency.circulatingSupply)} ${cryptocurrency.symbol.toUpperCase()}'),
            if (cryptocurrency.totalSupply != null)
              _buildStatItem(
                  'Total Supply',
                  '${NumberFormat("#,##0.00").format(cryptocurrency.totalSupply)} ${cryptocurrency.symbol.toUpperCase()}'),
            if (cryptocurrency.maxSupply != null)
              _buildStatItem(
                  'Max Supply',
                  '${NumberFormat("#,##0.00").format(cryptocurrency.maxSupply)} ${cryptocurrency.symbol.toUpperCase()}'),
          ],
        ),
      ),
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            if (details['links']?['homepage'] != null &&
                details['links']['homepage'][0].isNotEmpty)
              _buildLinkItem('Website', details['links']['homepage'][0]),
            if (details['links']?['blockchain_site'] != null &&
                details['links']['blockchain_site'][0].isNotEmpty)
              _buildLinkItem(
                  'Blockchain Explorer', details['links']['blockchain_site'][0]),
            if (details['links']?['whitepaper'] != null &&
                details['links']['whitepaper'].isNotEmpty)
              _buildLinkItem('White Paper', details['links']['whitepaper']),
            if (details['links']?['repos_url']?['github']?.isNotEmpty ?? false)
              _buildLinkItem(
                  'GitHub', details['links']['repos_url']['github'][0]),
            if (details['links']?['twitter_screen_name'] != null &&
                details['links']['twitter_screen_name'].isNotEmpty)
              _buildLinkItem('Twitter',
                  'https://twitter.com/${details['links']['twitter_screen_name']}'),
            if (details['links']?['facebook_username'] != null &&
                details['links']['facebook_username'].isNotEmpty)
              _buildLinkItem('Facebook',
                  'https://facebook.com/${details['links']['facebook_username']}'),
            if (details['links']?['subreddit_url'] != null &&
                details['links']['subreddit_url'].isNotEmpty)
              _buildLinkItem('Reddit', details['links']['subreddit_url']),
          ],
        ),
      ),
