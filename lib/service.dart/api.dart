...
class ApiService {
  ...
  // New method for fetching multiple cryptocurrencies by IDs
  static Future<List<Cryptocurrency>> getMultipleCryptocurrencies(List<String> ids) async {
    if (ids.isEmpty) return [];

    final cacheKey = 'multiple_cryptos_${ids.join('_')}';
    final cachedData = await CacheService.getCache(cacheKey);
    if (cachedData != null) {
      return (json.decode(cachedData) as List).map<Cryptocurrency>((json) => Cryptocurrency.fromJson(json)).toList();
    }
    ...
    try {
      // CryptoCompare: Fetch multiple symbols in one request
      final symbols = ids.map((id) => _idToSymbol[id] ?? id.toUpperCase()).join(',');
      final url = '${CryptoApiConfig.coinCompareBaseUrl}/pricemultifull?fsyms=$symbols&tsyms=USD';
      final data = await _makeRequest(url, headers: CryptoApiConfig.coinCompareHeaders);

      final List<Cryptocurrency> cryptos = [];
      for (var id in ids) {
        final symbol = _idToSymbol[id] ?? id.toUpperCase();
        final raw = data['RAW'][symbol]?['USD'];
        if (raw != null) {
          final crypto = Cryptocurrency(
            id: id.toLowerCase(),
            name: raw['FROMSYMBOL'], // Limited data; name might need adjustment
            symbol: symbol,
            price: raw['PRICE'].toDouble(),
            marketCap: raw['MKTCAP'].toDouble(),
            volume24h: raw['VOLUME24HOUR'].toDouble(),
            percentChange24h: raw['CHANGEPCT24HOUR'].toDouble(),
            image: 'https://www.cryptocompare.com${raw['IMAGEURL'] ?? '/media/37746251/btc.png'}', // Fallback image
      ...
      // CoinGecko fallback
      final baseUrlIndex = _random.nextInt(CryptoApiConfig.coinGeckoBaseUrls.length);
      final url = '${CryptoApiConfig.coinGeckoBaseUrls[baseUrlIndex]}/coins/markets?vs_currency=usd&ids=${ids.join(',')}&order=market_cap_desc&per_page=${ids.length}&page=1&sparkline=false';
      final data = await _makeRequest(url);
